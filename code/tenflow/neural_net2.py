...
def learn_model(xTrain,yTrain,hLayers,batchSize,numIters,saveMod,nodeType='sigmoid',bTerm=True,finalM=False):
    '''
    ...
    posIndices = np.where(yTrain == 1)[0]
    negIndices = np.where(yTrain == 0)[0]
    xTrainPos = xTrain[posIndices]
    yTrainPos = yTrain[posIndices]
    xTrainNeg = xTrain[negIndices]
    yTrainNeg = yTrain[negIndices]

    nxp,nxn = xTrainNeg.shape
    pxp,pxn = xTrainPos.shape
    ...
    with tf.Session() as sess:
            ...
            while (i < numIters): #and ((diff < tol) or (i < 500)):
                    i+=1
                    rand_indexP = np.random.choice(int(pxn), size=int(bSPos))
                    rand_indexN = np.random.choice(int(nxn), size=int(batchSize))
                    rand_x = np.vstack((xTrainNeg[rand_indexN,:],xTrainPos[rand_indexP,:]))
                    #rand_y = np.transpose([yTrain[rand_index,:]])
                    rand_y =np.vstack((yTrainNeg[rand_indexN,:],yTrainPos[rand_indexP,:]))
            ...
            allPreds = sess.run(model,feed_dict={x_data:xTrain,keep_prob:1.0})
	    disc_preds = sess.run(tf.greater(np.nan_to_num(allPreds),0.5)).astype(np.int64)
	    fpr, tpr, thresholds = metrics.roc_curve(np.nan_to_num(yTrain), np.nan_to_num(disc_preds), pos_label=1)
	    auc_val = metrics.auc(fpr, tpr)
            acc_val = metrics.accuracy_score(yTrain,disc_preds) 
    	    print("Full train auc val: "+str(auc_val))
	    print("Full train acc val: "+str(acc_val))
	    print("Mean preds: "+str(np.mean(allPreds)))
	    print("Std preds: "+str(np.std(allPreds)))
    sess.close() 
    prMod = model
