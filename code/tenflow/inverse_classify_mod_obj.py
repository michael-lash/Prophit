...
def invBackPro(var):
	...
	origObj = [invGraph.pred(np.hstack((np.hstack((var.x[var.uncInd][np.newaxis],var.optIndX)),var.modDirX)))[0][0]]
	origActObj = [valGraph.pred(np.hstack((np.hstack((var.x[var.uncInd][np.newaxis],var.optIndX)),var.modDirX)))[0][0]]
	...
	obj = [invGraph.pred(np.hstack((np.hstack((var.x[var.uncInd][np.newaxis],var.optIndX)),var.modDirX)))[0][0]+\
		np.linalg.norm(np.divide((var.estD - var.optx),(2*np.square(var.stdD))),2)*var.lam]
...
#np.multiply((var.lam/np.square(var.stdD)),np.square(np.linalg.norm(var.estD - var.optx,2)))]
	...
	with tf.Session() as sess:
		while ((iterN < var.mIts) and (diff > var.gTol)):
			...
			#dirGrad = invGraph.getInvGradG(var,dirGrad)
			dirGrad = invGraph.getInvGradG(var,dirGrad)
			...
			cObj = invGraph.pred(np.hstack((np.hstack((var.x[var.uncInd][np.newaxis],var.optIndX)),var.modDirX)))[0][0]+\
						np.linalg.norm(np.divide((var.estD - var.optx),(2*np.square(var.stdD))),2)*var.lam
						#np.square(np.linalg.norm(np.divide((var.estD - var.optx),(2*np.square(var.stdD))),2))*var.lam
			...
			while ((cObj > obj[-1]) and (var.gStep < 10000)):
				...
				###Need to update the gradient for optx
	                        ...
	                        #tmpx = (var.modDirX + (var.gStep*dirGradUp))[0] -- changed
        	                tmpx = (var.optx + (1/var.gStep*dirGradUp))
                	        #if var.doCausal == True:
                       		#       origTmpX = np.divide(tmpx[np.newaxis],var.probs.T)
                       		#       origTmpX = np.nan_to_num(origTmpX)
                        	#else:
			...
			origObj.append(invGraph.pred(np.hstack((np.hstack((var.x[var.uncInd][np.newaxis],var.optIndX)),var.modDirX)))[0][0])
			obj.append(invGraph.pred(np.hstack((np.hstack((var.x[var.uncInd][np.newaxis],var.optIndX)),var.modDirX)))[0][0]+\
					np.linalg.norm(np.divide((var.estD - var.optx),(2*np.square(var.stdD))),2)*var.lam)
					#np.square(np.linalg.norm(np.divide((var.estD - var.optx),(2*np.square(var.stdD))),2))*var.lam)
	...
	origActObj.append(valGraph.pred(np.hstack((np.hstack((var.x[var.uncInd][np.newaxis],var.optIndX)),var.modDirX)))[0][0])
	actObj.append(valGraph.pred(np.hstack((np.hstack((var.x[var.uncInd][np.newaxis],var.optIndX)),var.modDirX)))[0][0]+\
			np.linalg.norm(np.divide((var.estD - var.optx),(2*np.square(var.stdD))),2)*var.lam)
			#np.square(np.linalg.norm(np.divide((var.estD - var.optx),(2*np.square(var.stdD))),2))*var.lam)
	...
	return var, obj, actObj, origObj, origActObj
