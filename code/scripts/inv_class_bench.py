...
def main():
    ...
    logFile = projDir+"results/benchIC--noprobopt--log--sig.txt"
    logIt = Logger(logFile)
    #resultsFileD1 = projDir+"results/cvResultsD1.pickle" #Where the cross-validation results will be saved
    invModel = projDir+"model/D1/cvModelD1.ckpt"
    indModel = projDir+"model/Ind/INDcvModelD1.ckpt"
    valModel = projDir+"model/D2/cvModelD2.ckpt"

    resultsFileD2 = projDir+"results/NOCAUSbenchICResults--noprobopt--sig.pickle" #Where the cross-validation results will be saved
    ...
    with open(resultsFileD2, 'w') as f:
            pickle.dump({'xIndDir':xDict, 'obj':objMat, 'budgets':budget_list, 'optObj':optObjMat}, f)
