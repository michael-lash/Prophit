...
from inverse_classify_mod_obj import invBackPro
...
def main():
    ...
    logFile = projDir+"results/CAUSEbenchIC--probopt--aug--lampoint9-new--log.txt"
    ...
    resultsFileD2 = projDir+"results/CAUSEbenchICResults--probopt--aug--lampoint0001-Gnorm.pickle" #Where the cross-validation results will be saved
    ...
    objModMat = np.zeros((nx,(len(budget_list)+1)))
    optModObjMat = np.zeros((nx,(len(budget_list)+1)))
	...
	for j in range(len(budget_list)):
		...
		logIt.startTime()
		icVar,obj,objAct,modObj,modActObj = invBackPro(icVar)
		logIt.finishTime()
		...
		if j == 0:
			objMat[i,j] = objAct[0]
			optObjMat[i,j] = obj[0]
			objModMat[i,j] = modActObj[0]
			optModObjMat[i,j] =modObj[0]
			probDict[0][i] = icVar.origProbs.ravel()

		objMat[i,j+1] = objAct[-1]
		optObjMat[i,j+1] = obj[-1]
		objModMat[i,j] = modActObj[-1]
                optModObjMat[i,j] =modObj[-1]
    ...
    with open(resultsFileD2, 'w') as f:
            pickle.dump({'xIndDir':xDict, 'obj':objMat, 'budgets':budget_list, 'optObj':optObjMat, 'modObj':objModMat, 'optModObj':optModObjMat, 'apsDict':probDict}, f)
